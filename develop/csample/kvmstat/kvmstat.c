...
void refresh_kvm_trace()
{
    ...
    for(i = 0; i < cpus; i++) {
        ...
        {
            fprintf(stderr, "Read trace fd error.\n");
        }
        for(j = 0; j < ntrace; j++) {
            trace_v->value[j] = (long long *)tmp_values[j+1];
            fprintf(stderr, "==ntrace is %d  value%d is %lld indom %d %s\n", j, i, trace_v->value[j], trace_indom, cpu);
	}
...
fprintf(stderr, "%s %d\n", trace_nametab[i], i);
                    ...
                    if (fd == -1) {
	                fprintf(stderr, "perf_event_open error!\n");
	                sts = 1;
                    }
                    if (flag == 0) {
                        group_fd[cpu] = fd;
...
kvm_fetchCallBack(pmdaMetric *mdesc, unsigned int inst, pmAtomValue *atom)
{
	...
	fprintf(stderr, "item %d,  %lld name %s\n", pmID_item(*idp),  va->value[pmID_item(*idp)], name);
	atom->ll = (__int64_t)va->value[pmID_item(*idp)];
        break;
    case CLUSTER_DEBUG:
        switch (pmID_item(*idp)) {
        case 0:	/* kvmstat.efer_reload */
	    atom->ll = kvmstat.debug[0];
	    break;
        case 1:	/* kvmstat.exits */
	    atom->ll = kvmstat.debug[1];
	    break;
        case 2:	/* kvmstat.fpu_reload */
	    atom->ll = kvmstat.debug[2];
	    break;
        case 3:	/* kvmstat.halt_attempted_poll */
	    atom->ll = kvmstat.debug[3];
	    break;
        case 4:	/* kvmstat.halt_exits */
	    atom->ll = kvmstat.debug[4];
	    break;
        case 5:	/* kvmstat.halt_successful_poll */
	    atom->ll = kvmstat.debug[5];
	    break;
        case 6:	/* kvmstat.halt_wakeup */
	    atom->ll = kvmstat.debug[6];
	    break;
        case 7:	/* kvmstat.host_state_reload */
	    atom->ll = kvmstat.debug[7];
	    break;
        case 8:	/* kvmstat.hypercalls */
	    atom->ll = kvmstat.debug[8];
	    break;
        case 9:	/* kvmstat.insn_emulation */
	    atom->ll = kvmstat.debug[9];
	    break;
        case 10:/* kvmstat.insn_emulation_fail */
	    atom->ll = kvmstat.debug[10];
	    break;
        case 11:/* kvmstat.invlpg */
	    atom->ll = kvmstat.debug[11];
	    break;
        case 12:	/* kvmstat.io_exits */
	    atom->ll = kvmstat.debug[12];
	    break;
        case 13:	/* kvmstat.irq_exits */
	    atom->ll = kvmstat.debug[13];
	    break;
        case 14:	/* kvmstat.irq_injections */
	    atom->ll = kvmstat.debug[14];
	    break;
        case 15:	/* kvmstat.irq_window */
	    atom->ll = kvmstat.debug[15];
	    break;
        case 16:	/* kvmstat.largepages */
	    atom->ll = kvmstat.debug[16];
	    break;
        case 17:	/* kvmstat.mmio_exits */
	    atom->ll = kvmstat.debug[17];
	    break;
        case 18:	/* kvmstat.mmu_cache_miss */
	    atom->ll = kvmstat.debug[18];
	    break;
        case 19:	/* kvmstat.mmu_flooded */
	    atom->ll = kvmstat.debug[19];
	    break;
        case 20:	/* kvmstat.mmu_pde_zapped */
	    atom->ll = kvmstat.debug[20];
	    break;
        case 21:	/* kvmstat.mmu_pte_updated */
	    atom->ll = kvmstat.debug[21];
	    break;
        case 22:	/* kvmstat.mmu_pte_write */
	    atom->ll = kvmstat.debug[22];
	    break;
        case 23:	/* kvmstat.mmu_recycled */
	    atom->ll = kvmstat.debug[23];
	    break;
        case 24:	/* kvmstat.mmu_shadow_zapped */
	    atom->ll = kvmstat.debug[24];
	    break;
        case 25:	/* kvmstat.mmu_unsync */
	    atom->ll = kvmstat.debug[25];
	    break;
        case 26:	/* kvmstat.nmi_injections */
	    atom->ll = kvmstat.debug[26];
	    break;
        case 27:	/* kvmstat.nmi_window */
	    atom->ll = kvmstat.debug[27];
	    break;
        case 28:	/* kvmstat.pf_fixed */
	    atom->ll = kvmstat.debug[28];
	    break;
        case 29:	/* kvmstat.pf_guest */
	    atom->ll = kvmstat.debug[29];
	    break;
        case 30:	/* kvmstat.remote_tlb_flush */
	    atom->ll = kvmstat.debug[30];
	    break;
        case 31:	/* kvmstat.request_irq */
	    atom->ll = kvmstat.debug[31];
	    break;
        case 32:	/* kvmstat.signal_exits */
	    atom->ll = kvmstat.debug[32];
	    break;
        case 33:	/* kvmstat.tlb_flush */
...
kvmstat_init(pmdaInterface *dp)
{
	...
	pmsprintf(name, sizeof(name),
			"kvmstat.trace.%s",  trace_nametab[m]);
       fprintf(stderr, "%s-----%s\n", name, trace_nametab[m]);
...
main(int argc, char **argv)
{
    ...
    snprintf(helppath, sizeof(helppath), "%s%c" "kvmstat" "%c" "help",
		pmGetConfig("PCP_PMDAS_DIR"), sep, sep);
    pmdaDaemon(&dispatch, PMDA_INTERFACE_6, pmGetProgname(), KVMSTAT, "kvmstat.log", helppath);
