...
then
  ...
  do
    ...
    do
      ...
      for k in `seq 1 $tmep`
      do
        cat $RESULT_DIR/fio-${disk}-$i$k.txt | grep iops | awk -F ',' '{ print $3 }' | awk -F '=' '{ print $2 }'  >> $GETDATA_DIR/fio-${disk}-$i$k.csv
        if (( $agent == $TESTTIMES ))
        then
          cat $GETDATA_DIR/fio-${disk}-$i$k.csv | awk '{ sum += $1; } END { print sum/NR}' > $GETDATA_DIR/aver-fio-${disk}-$i$k.csv
          cat $GETDATA_DIR/aver-fio-${disk}-$i$k.csv >> $GETDATA_DIR/tmpsum-$tazyu.csv
          rm -rf $GETDATA_DIR/fio-${disk}-$i$k.csv
          rm -rf $GETDATA_DIR/aver-fio-${disk}-$i$k.csv
        fi
        ((tmp++))
        echo $tmp
      done
